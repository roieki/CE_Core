...
switch ($action){
	...
	case 'get_fxp_forums':
		$links = $_POST['links'];
		get_fxp_forums($links);
		break;
	case 'get_forums_list':
		get_forums_list();
		break;
	default:
	break;
...
function get_forums_list(){
	...
	$result = $mysqli->query('select * from forums_list');
	$table = array();
	while ($row = $result->fetch_assoc()){
		if (!($index[md5($row['forum_name'])])){
			$item = '';
			$item['label'] = $row['forum_name'];
            $item['id'] = $row['forumid'];
            $table[] =$item;
		}	
		$index[md5($row['forum_name'])] = $row['forumid'];
	}
	$result->forum_list = $table;
	$result->forum_index = $index;
