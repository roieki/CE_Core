...
if (!empty($_POST['submit']) || !empty($_POST['submit2'])) {
  ...
  if ($_POST['submit']) {
    ...
    try {
      if ($files) {
        foreach ($files as $file) {
          $adgroup_spec[basename($file['name'])] = '@'.$file['tmp_name'];
        }
      }
      $result = $facebook->api($act_id . '/adgroups', 'post', $adgroup_spec);
      if (!empty($result['id'])) {
    ...
    try {
      $currency_code = 'USD';
      $targeting_stats = $facebook->api(
                           $act_id . '/reachestimate',
                           'get',
                           array('targeting_spec' => $targeting,
