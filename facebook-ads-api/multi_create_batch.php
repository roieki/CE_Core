...
if (!empty($_POST['submit']) || !empty($_POST['submit2'])) {
  ...
  // handle batch param special case for multiple same named attached_files
  $used_image_names = array();
  foreach ($files as $file) {
    $img_name_tmp = substr(basename($file['tmp_name']),
                           0,
                           strpos(basename($file['tmp_name']), '.'));
    if (isset($used_image_names[$img_name_tmp])) {
      $used_image_names[$img_name_tmp] += 1;
    } else {
      $used_image_names[$img_name_tmp] = 1;
    }
    $num = $used_image_names[$img_name_tmp];
    $img_name_no_ext[$img_name_tmp.$num] = $file['tmp_name'];
  }
  ...
  if ($_POST['submit']) {
    ...
    try {
      ...
      $used_image_names = array();
      $batch_specs = array();
      foreach ($adgroup_specs as $ad_spec) {
        $graph_method = $act_id . '/adgroups';

        // get ad specs
        $ad_params = "";
        foreach ($ad_spec as $p => $v) {
          $val        = is_array($v) ? json_encode($v) : $v;
          $param_val  = $p . '=' . $val;
          $ad_params .= empty($ad_params) ? $param_val : '&' . $param_val;
        }

        // get attached file name
        $img_name_tmp = substr($image_file_name[$count],
                               0,
                               strpos($image_file_name[$count], '.'));
        if (isset($used_image_names[$img_name_tmp])) {
          $used_image_names[$img_name_tmp] += 1;
        } else {
          $used_image_names[$img_name_tmp] = 1;
        }
        $attached_img_name = $img_name_tmp . $used_image_names[$img_name_tmp];

        // build batch specs
        $batch_specs[] = array(
                           'method'         => 'post',
                           'relative_url'   => $graph_method . '?' . 
                                               $ad_params,
                           'attached_files' => $attached_img_name);
        $count++;
      }
      $params = array('batch' => json_encode($batch_specs));

      // add on files
      foreach ($img_name_no_ext as $file_name=>$tmp_name) {
        $params[$file_name] = '@' . $tmp_name;
      }
    ...
    $graph_call = $act_id . '/reachestimate';
    try {
      $currency_code = 'USD';
      $batch_specs   = array();
      foreach ($adgroup_specs as $ad_spec) {
        $graph_method  = $act_id . '/reachestimate';
        $target_spec   = 'targeting_spec=' . json_encode($ad_spec['targeting']);
      ...
      $targeting_stats = $facebook->api('', '', $params); 
    } catch (Exception $e) {
      $error_message = $e->getMessage();
    }
  }
...
if ($error_message) {
  ...
  if ($graph_call) {
    echo 'Graph Call: <pre>' . $graph_call . '</pre><br>';
  }
  echo 'Request: <pre>' . var_export($adgroup_specs, true) . '</pre><br>';
  if ($results) {
    echo 'Response: <pre>' . var_export($results, true) . '</pre>';
  }
...
echo '
<form action="multi_create_batch.php" method="post" enctype="multipart/form-data">
<table>
...
<tr><td colspan=3 style="background-color: #aaaaaa;"><h3>Targeting</h3></td></tr>
...
<tr><td colspan=3 style="background-color: #aaaaaa;"><h3>Campaigns and Pricing</h3></td></tr>
<tr><td>Campaign:</td><td><select name="campaign_id_1">';
foreach ($campaigns['data'] as $campaign) {
  $campaign_id_tmp = trim(sprintf('%20.0f', $campaign['campaign_id']));
  echo '<option value="' . $campaign_id_tmp . '"' . ($campaign_id == $campaign_id_tmp ? ' selected' : '') . '>' . $campaign['name'] . '</option>';
}
echo '</select></td></tr>
