...
function getStatusString($status) {
  ...
  case AdGroupStatus::CAMPAIGN_PAUSED:
    return 'paused: autoresumes when campaign unpaused.<br/><br/>*pause to prevent autoresuming.';
    break;
...
function getCampaignStat($stats, $id) {
  foreach ($stats['data'] as $stat) {
    $stat_id = sprintf("%20.0f", $stat['campaign_id']);
    if ($stat_id == $id) {
      return $stat;
    }
  }
...
function getAdgroupStat($stats, $id) {
  foreach ($stats['data'] as $stat) {
    $stat_id = sprintf("%20.0f", $stat['adgroup_id']);
    if (($stat_id == $id)) {
      return $stat;
    }
  }
